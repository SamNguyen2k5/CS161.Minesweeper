...
ScreenGameLoseSidePanel::ScreenGameLoseSidePanel(Minefield* minefield):
    ScreenGameSidePanel(minefield)
{
    ...
    background_panel.setFillColor(context.COLOUR_BACKGROUND_LOSE);
    ...
    text_status.setString("YOU LOSE!");
...
void ScreenGameLoseSidePanel::update_graphics() {
    ScreenGameSidePanel::update_graphics();
}

void ScreenGameLoseSidePanel::draw(sf::RenderTarget &target, sf::RenderStates states) const {
...
void ScreenGameLoseSidePanel::update_on_event(sf::Event event) {
    ScreenGameSidePanel::update_on_event(event);

    btn_back.update_on_event(event);
}

bool ScreenGameLoseSidePanel::ask_is_back() { return btn_back.ask_is_clicked(); }
